...
(function() {
    ...
    let debugLog = [];
    const SETTINGS_KEY = 'gemini_ocr_settings_v23_bestfit_pc_parserfix';
    ...
    const logDebug = (message) => { if (!settings.debugMode) return; const timestamp = new Date().toLocaleTimeString(); const logEntry = `[${timestamp}] ${message}`; console.log(`[OCR BestFit] ${logEntry}`); debugLog.push(logEntry); document.dispatchEvent(new CustomEvent('ocr-log-update')); };
    ...
    function manageContainer(container) { if (managedContainers.has(container)) return; logDebug(`New container found. Managing: ${container.className}`); container.querySelectorAll('img').forEach(observeImageForSrcChange); imageObserver.observe(container, { childList: true, subtree: true }); managedContainers.set(container, true); }
    ...
    function observeImageForSrcChange(img) { const processTheImage = (src) => { if (src?.includes('/api/v1/manga/')) { primeImageForOcr(img); return true; } return false; }; if (processTheImage(img.src)) return; if (attachedAttributeObservers.has(img)) return; const attributeObserver = new MutationObserver((mutations) => { for (const mutation of mutations) if (mutation.attributeName === 'src') if (processTheImage(img.src)) { attributeObserver.disconnect(); attachedAttributeObservers.delete(img); break; } }); attributeObserver.observe(img, { attributes: true }); attachedAttributeObservers.set(img, attributeObserver); }
    function primeImageForOcr(img) { if (managedElements.has(img)) return; const process = () => { if (managedElements.has(img)) return; img.crossOrigin = "anonymous"; const realSrc = img.src; if (ocrCache.get(img) === 'pending') return; processImage(img, realSrc); }; if (img.complete && img.naturalHeight > 0) process(); else img.addEventListener('load', process, { once: true }); }

    // --- CORRECTED: processImage now handles direct array response ---
        ...
        GM_xmlhttpRequest({
            ...
            onload: (res) => {
                try {
                    ...
                    // Add a check to ensure we received a valid array, like in the mobile script
                    if (!Array.isArray(data)) {
                        throw new Error('Server response was not a valid OCR data array.');
                    }
    ...
    function findBestFitFontSize(text, availableWidth, availableHeight, isVertical) { if (!text || availableWidth <= 0 || availableHeight <= 0) return 0; measurementSpan.style.writingMode = isVertical ? 'vertical-rl' : 'horizontal-tb'; measurementSpan.textContent = text; let low = 6, high = 200, bestSize = 6; while (low <= high) { const mid = Math.floor((low + high) / 2); if (mid <= 0) break; measurementSpan.style.fontSize = `${mid}px`; if (measurementSpan.offsetWidth <= availableWidth && measurementSpan.offsetHeight <= availableHeight) { bestSize = mid; low = mid + 1; } else { high = mid - 1; } } return bestSize; }
    function calculateAndApplyOptimalStyles(overlay) {
        ...
        overlay.querySelectorAll('.gemini-ocr-text-box').forEach(box => {
            ...
            else if (settings.textOrientation === 'forceHorizontal') { isVertical = false; finalFontSize = findBestFitFontSize(text, availableWidth, availableHeight, false); }
            else { // Smart mode
        ...
        try {
            ...
            for (const [img, state] of managedElements.entries()) {
                ...
                if (state.lastWidth !== rect.width || state.lastHeight !== rect.height) {
                    calculateAndApplyOptimalStyles(state.overlay);
    ...
    // --- Unchanged UI, Batch, and Init Functions ---
    ...
    function createMeasurementSpan() { if (measurementSpan) return; measurementSpan = document.createElement('span'); measurementSpan.style.cssText = `position: absolute !important; visibility: hidden !important; left: -10000px; top: -10000px; height: auto !important; width: auto !important; white-space: nowrap !important; z-index: -1 !important;`; document.body.appendChild(measurementSpan); }
    async function init() { const loadedSettings = await GM_getValue(SETTINGS_KEY); if (loadedSettings) { try { const parsedSettings = JSON.parse(loadedSettings); settings = { ...settings, ...parsedSettings }; } catch(e) { logDebug("Could not parse saved settings. Using defaults."); } } createUI(); bindUIEvents(); applyDynamicStyles(); createMeasurementSpan(); UI.serverUrlInput.value = settings.ocrServerUrl; UI.imageServerUserInput.value = settings.imageServerUser || ''; UI.imageServerPasswordInput.value = settings.imageServerPassword || ''; UI.ankiUrlInput.value = settings.ankiConnectUrl; UI.ankiFieldInput.value = settings.ankiImageField; UI.debugModeCheckbox.checked = settings.debugMode; UI.interactionModeSelect.value = settings.interactionMode; UI.textOrientationSelect.value = settings.textOrientation; UI.colorThemeSelect.value = settings.colorTheme; UI.dimmedOpacityInput.value = settings.dimmedOpacity * 100; UI.fontMultiplierHorizontalInput.value = settings.fontMultiplierHorizontal; UI.fontMultiplierVerticalInput.value = settings.fontMultiplierVertical; UI.sitesConfigTextarea.value = settings.sites.map(s => [s.urlPattern, s.overflowFixSelector, ...(s.imageContainerSelectors || [])].join('; ')).join('\n'); setInterval(manageScrollFix, 500); activateScanner(); observeChapters(); if (!overlayUpdateRunning) requestAnimationFrame(updateAllOverlays); }
